...
instfreqfeat = cellfun(@(x) isnan1(x),XTrainN_inst,'UniformOutput',false);
clear XV_inst;
clear XTrainN_inst;
...
pentropyfeat = cellfun(@(x) isnan1(x),XTrainN_pen,'UniformOutput',false);
clear XV_pen;
clear XTrainN_pen;


save('mu_sg.mat','mu_sc','mu_pen','mu_inst','sg_sc','sg_pen','sg_inst','-v7.3');
...
for i=1:length(lead_sets)
    ...
    [leads, leads_idx] = get_leads(header_data,num_leads);
     pentropyfeat_leads=cellfun(@(x) x(leads_idx,:),pentropyfeat,'UniformOutput',false);
     instfreqfeat_leads=cellfun(@(x) x(leads_idx,:),instfreqfeat,'UniformOutput',false);
     scatt_feat_leads=cellfun(@(x) scattering_features_arrange(x,leads_idx),scatt_feat,'UniformOutput',false);   
...
%     model = mnrfit(Features_leads,label,'model','hierarchical');
      model=model_formation(pentropyfeat_leads, instfreqfeat_leads, scatt_feat_leads, lab_new);
